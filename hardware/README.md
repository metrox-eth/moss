...
Design in Fusion 360. The current prototype is **V0.3**, being assembled from its first print set (111 pieces, see `docs/printed_parts.md`). Fit problems are logged in `docs/assembly_feedback.md`: some are fixed on the prototype, the others wait for the next hardware revision, which has no number yet.
